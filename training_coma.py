...
if __name__== '__main__':
    parser = argparse.ArgumentParser(description='Train Coma')
    ...
    for epoch in range(epochs):
        ...
        with tqdm(training_loader, unit="batch") as tepoch:
            ...
            for vertices, emotion, _, _, template, num_frames in tepoch:
            ...
            with torch.no_grad():
                with tqdm(val_loader, unit='batch') as vepoch:
                    for vertices, emotion, name, obj, template, num_frames in vepoch:
                        ...
                        output = model.predict(emotion,vertices[:,0,:],template,(num_frames-1)).to(device)
                        output = output.view(vertices.shape[0],num_frames,int(args.vertices_dim),3)
                        vertices = vertices.view(vertices.shape[0],num_frames,int(args.vertices_dim),3)
                        loss = composite_loss(output[:,1:,:,:], vertices[:,1:,:,:], float(args.c1), float(args.c2))
